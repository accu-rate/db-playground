-- Aufenthaltsdauer
WITH PedDurations AS (SELECT pedID,
                             MIN(time)               AS entry_time,
                             MAX(time)               AS exit_time,
                             (MAX(time) - MIN(time)) AS duration_seconds
                      FROM ${selectedTable}
                      GROUP BY pedID),
     DurationBuckets AS (SELECT pedID, duration_seconds, CEIL(duration_seconds / 60) AS minute_bucket
                         FROM PedDurations)
SELECT minute_bucket AS Minuten,
       COUNT(*)      AS 'Anzahl Personen'
...
WITH InArea AS (SELECT *
                FROM ${selectedTable}
                WHERE posX BETWEEN ${posXMin} AND ${posXMax}
                  AND posY BETWEEN ${posYMin} AND ${posYMax}),
     PedDurations AS (SELECT pedID,
                             MIN(time)               AS entry_time,
                             MAX(time)               AS exit_time,
                             (MAX(time) - MIN(time)) AS duration_seconds
                      FROM InArea
...
FROM ${selectedTable}
WHERE posX BETWEEN ${posXMin}
  AND ${posXMax}
  AND posY BETWEEN ${posYMin}
  AND ${posYMax}
GROUP BY time
ORDER BY time ASC;
...
WHERE time IS NOT NULL
    ...
    , Velocity AS (
SELECT
    current_time, pedID, SQRT(POWER(posX - prev_posX, 2) + POWER(posY - prev_posY, 2)) / (current_time - prev_time) AS speed
FROM LaggedData
WHERE prev_time IS NOT NULL
    )
    , AverageSpeed AS (
SELECT
    current_time, pedID, AVG (speed) AS avg_speed
FROM Velocity
...
SELECT current_time AS 'Zeit [s]', COUNT(pedID) AS 'Anzahl Personen im Stau'
FROM Filtered
GROUP BY current_time
ORDER BY current_time;

-- Anzahl Ausgänge vs. Räumungszeit
WITH AvailableExits AS (SELECT variant,
                               COUNT(*) AS available_exits
                        FROM variantmapping
                        WHERE assignment = 'true'
                        GROUP BY variant)
SELECT a.available_exits AS 'Anzahl Ausgänge', b.value AS Raeumungszeit
FROM AvailableExits a
         JOIN
     variantresultsummary b
     ON
         a.variant = b.variant
WHERE b."constraint type" = 'evacuationTime'
ORDER BY b.value, a.available_exits;

-- Anzahl Personen vs. Räumungszeit
WITH NoOfPeds AS (SELECT variant,
                         SUM(assignment) AS no_of_peds
                  FROM variantmapping
                  WHERE CAST(assignment AS INTEGER) < ${noOfPeds}
                  GROUP BY variant)
SELECT b.value           AS Raeumungszeit,
